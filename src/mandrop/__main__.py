...
To study a buffer/surfactant composition: edit `params.physical.*` (sigma,
viscosities, flow rates). To switch resolution: edit `params.sim.resolution_um`.
"""
...
def main():
    params = Params(
        ...
        sim     =SimulationParams(resolution_um=1.0),
    )
    geo, step, (f0, phi0), lat = build(params)
    p_geo = geo["params"]
    ...
    print(f"{'step':>8} | {'MLUPS':>6} | {'max|u| (m/s)':>12} | {'Δp (Pa)':>9} | {'drops':>5} {'d_mean (µm)':>11} {'CV':>6} | {'f (Hz)':>8}")
    print("-" * 100)

    def update_plots(f_c, phi_c, probe_history, step_num, wall_dt):
