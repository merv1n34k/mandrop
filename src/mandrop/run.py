...
def run(step, f0, phi0, interior, params,
    ...
    on_chunk(f, phi, probe_history, step_num, dt) is called after each chunk.
    Return False from on_chunk to stop early.

    Returns (f_final, phi_final, total_steps).
    """
    ...
    while chunk < n_chunks:
        ...
        if on_chunk is not None:
            if on_chunk(f_c, phi_c, probe_history, step_num, dt) is False:
                break
    ...
    return f_final, phi_final, total_steps
